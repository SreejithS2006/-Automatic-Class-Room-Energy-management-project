...
void loop() {
  if (Firebase.ready() && signupOK && (millis() - sendDataPrevMillis > 5000 || sendDataPrevMillis == 0)) {
    ...
    if (random(0, 100) > 90) isOccupied = !isOccupied;

    // Send Temperature to Firebase
    if (Firebase.RTDB.setFloat(&fbdo, "classroom/temperature", temperature)) {
      Serial.print("Temp Sent: "); Serial.println(temperature);
    } else {
      Serial.println("FAILED: " + fbdo.errorReason());
    }

    // Send Occupancy Status to Firebase
    if (Firebase.RTDB.setBool(&fbdo, "classroom/occupied", isOccupied)) {
      Serial.print("Occupancy Sent: "); Serial.println(isOccupied ? "YES" : "NO");
    } else {
      Serial.println("FAILED: " + fbdo.errorReason());
    }
  }
